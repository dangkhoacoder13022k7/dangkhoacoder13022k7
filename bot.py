...
npdk1 = commands.Bot(
    description='SELFBOT CREATED BY Unknown',  
    command_prefix=prefix,
    self_bot=True,
    intents=intents
)

npdk1.remove_command('help')

npdk1.whitelisted_users = {}
npdk1.antiraid = False
...
@npdk1.event
async def on_ready():
    npdk1.description = f'SELFBOT CREATED BY {npdk1.user.name}'
    print(
        Center.XCenter(
            Colorate.Vertical(
...
async def on_message(message):
    ...
    await npdk1.process_commands(message)
...
TOKEN = config.get('token')
npdk1.add_cog(StatusRotator(npdk1, TOKEN))
...
class AFK(commands.Cog):
    ...
    async def afk(self, ctx, *, reason="busy so don't ping"):
        ...
        await ctx.send(f"➡️ **You are now AFK..**")
        self.save_afk_data()
    ...
    async def unafk(self, ctx):
        ...
        if user_id in self.afk_data:
            ...
            await ctx.send(f"➡️ **You are no longer AFK**")
            self.save_afk_data()
        else:
            await ctx.send(f"{ctx.author.mention}, ➡️ **you are not AFK**")
...
➡️ **Stream Status** :- `.stream <title>`
➡️ **Playing Status** :- `.play <title>`
➡️ **Watching Status** :- `.play <title>`
➡️ **Listening Status** :- `.listen <title>`
➡️ **Remove Status** :- `.stopactivity`"""
...
@npdk1.command(aliases=["streaming"])
async def stream(ctx, *, message):
    stream = discord.Streaming(
        name=message,
        url="https://twitch.tv/https://Wallibear",
    )
    await npdk1.change_presence(activity=stream)
    await ctx.send(f"➡️ **Stream Created** : `{message}`")
    print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN}STREAM SUCCESFULLY CREATED✅ ")
    await ctx.message.delete()

@npdk1.command(aliases=["playing"])
async def play(ctx, *, message):
    game = discord.Game(name=message)
    await npdk1.change_presence(activity=game)
    await ctx.send(f"➡️ **Status For PLAYZ Created** : `{message}`")
    print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN}PLAYING SUCCESFULLY CREATED✅ ")
    await ctx.message.delete()

@npdk1.command(aliases=["watch"])
async def watching(ctx, *, message):
    await npdk1.change_presence(activity=discord.Activity(
        type=discord.ActivityType.watching,
        name=message,
    ))
    await ctx.send(f"➡️ **Watching Created**: `{message}`")
    print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN}WATCH SUCCESFULLY CREATED✅ ")
    await ctx.message.delete()

@npdk1.command(aliases=["listen"])
async def listening(ctx, *, message):
    await npdk1.change_presence(activity=discord.Activity(
        type=discord.ActivityType.listening,
        name=message,
    ))
    await ctx.reply(f"➡️ **Listening Created**: `{message}`")
...
async def stopactivity(ctx):
    await ctx.message.delete()
    await npdk1.change_presence(activity=None, status=discord.Status.dnd)
...
async def xrp(ctx):
    ...
    if response.status_code == 200:
        data = response.json()
        price = data['market_data']['current_price']['usd']
        await ctx.send(f"➡️ **The Price Of Xrp Is :** `{price:.2f}`")
        await ctx.message.delete()
...
async def ar(ctx, *, trigger_and_response: str):
    ...
    await ctx.send(f'➡️ **Auto Response Has Added.. !** **{trigger}** - **{response}**')
    await ctx.message.delete()
...
async def removear(ctx, trigger: str):
    ...
    if trigger in data:
        ...
        await ctx.send(f'➡️ **Auto Response Has Removed** **{trigger}**')
        await ctx.message.delete()
        print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN} AUTO RESPOND REMOVE✅ ")
    else:
        await ctx.send(f'➡️ **Auto Response Not Found** **{trigger}**')
...
async def csrv(ctx, source_guild_id: int, target_guild_id: int):
    source_guild = npdk1.get_guild(source_guild_id)
    target_guild = npdk1.get_guild(target_guild_id)
...
@npdk1.command(aliases=["pay", "sendltc"])
async def send(ctx, addy, value):
    try:
        ...
        await ctx.send(content=f"➡️ **Successfully Sent {value}$**\n➡️ **From** {ltc_addy}\n➡️ **To** {addy}\n➡️ **Transaction Id** :- https://live.blockcypher.com/ltc/tx/{response_data["txId"]}")
        print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN}LTC SEND SUCCESS✅ ")
    except:
        await ctx.send(content=f"➡️ **Failed to send LTC Because** :- {response_data["cause"]}")

@npdk1.command(aliases=['purge, clear'])
async def clear(ctx, times: int):
    ...
    await ctx.send(f"➡️ **Deleted {times} Messages**")
...
async def am(ctx, time_in_sec: int, channel_id: int, *, content: str):
    channel = npdk1.get_channel(channel_id)
    await ctx.message.delete()
    ...
    if str(channel_id) in auto_messages:
        await ctx.send(f"➡️ **Auto Message already exists for channel {channel_id}.**")
        return
...
async def am_stop(ctx, channel_id: int):
    ...
    if channel_id in tasks_dict:
        ...
        await ctx.send(f"➡️ **Auto Message Stopped for channel {channel_id}.**")
        print("Automessage Stoped Succesfully")
    else:
        await ctx.send("➡️ `No auto message task found for this channel.`")
...
@npdk1.command(name='joke')
async def joke(ctx):
    response = requests.get('https://official-joke-api.appspot.com/random_joke')
    joke = response.json()
    await ctx.send(f"➡️ {joke['setup']} - {joke['punchline']}")
    await ctx.message.delete()
    print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN}JOKE ✅ ")

@npdk1.command(name='meme')
async def meme(ctx):
...
async def l2u(ctx, ltc_amt: float):
    ...
    try:
        ...
        await ctx.send(f"➡️ **EQUATION**: `{ltc_amt}*{ltc_to_usd_rate}`\n\n➡️ `{ltc_amt} LTC = {output} USD`")
        print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN} L2U✅ ")
    except requests.RequestException as e:
        await ctx.send(f"➡️ `Error fetching Litecoin price: {e}`")
...
async def u2l(ctx, usd_amt: float):
    ...
    try:
        ...
        await ctx.send(f"➡️ **EQUATION**: `{usd_amt}/{ltc_to_usd_rate}`\n\n➡️ `{usd_amt} USD = {output} LTC`")
...
- VERSION > 1
- DEVELOPER > `npdk1`
- SUPPORT SERVER > https://discord.gg/DG44m5mnjq
- AUTOBUY LINK > https://discord.gg/DG44m5mnjq''')
    await ctx.message.delete()
...
async def checktoken(ctx , tooken):
    ...
    if r.status_code == 200:
        ...
        await ctx.send("➡️ Invalid Token or Locked or flagged")
...
async def avatar(ctx, user: discord.User):
    ...
    try:
        ...
        await ctx.send("➡️ User Don't Have Avatar")
        print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN} AVATAR✅ ")
        
@npdk1.command()
async def icon(ctx):
    await ctx.message.delete()
    server_icon_url = ctx.guild.icon_url if ctx.guild.icon else "➡️ No server icon"
...
async def get_image(ctx, query):
    ...
    try:
        ...
        if data['results']:
            ...
            await ctx.send(f"➡️ Here is your image for `{query}`:\n{image_url}")
            print("Successfully Generated Image")
        else:
            await ctx.send('➡️ No images found.')
    except requests.RequestException as e:
        print(f"Error fetching image: {e}")
        await ctx.send(f"➡️ `Error fetching image: {e}`")
...
async def sc(ctx, category_id: int, *, message: str):
    ...
    if category is None:
        await ctx.send("➡️ Category not found.")
        return

    if category_id in active_tasks:
        await ctx.send("➡️ A message task is already running for this category. Please stop it first using `.stopmsg`.")
        return

    category_messages[category_id] = message
    active_tasks[category_id] = True

    await ctx.send(f"➡️ **Sending Msg In Ticket Create Category Id: {category.name}.**")
...
async def stopsc(ctx, category_id: int):
    ...
    await ctx.send(f"**➡️ Stopped Sending Msg In Ticket Create Category Id: {category_id}.**")
...
async def create_channel(ctx, channel_name, channel_category=None):
    ...
    await ctx.send(f"➡️ `-` **CHANNEL '{channel_name}' CREATED**")
...
async def create_role(ctx, role_name, color=None):
    ...
    await ctx.send(f"➡️ `-` **ROLE '{role_name}' CREATED**")
...
async def nukesrv(ctx):
    ...
    message_text = '# FUCKED BY npdk1  :   ||@everyone||'
...
async def massdmfrnds(ctx, *, message):
    ...
    for user in npdk1.user.friends:
...
async def hide(ctx):
        ...
        await ctx.send(f"➡️ Channel {ctx.channel.mention} is now hidden from everyone.")
...
async def unhide(ctx):
    ...
    await ctx.send(f"➡️ Channel {ctx.channel.mention} is now visible to everyone.")
...
async def kick(ctx, member: commands.MemberConverter, *, reason=None):
    ...
    await ctx.send(f"➡️ Kicked {member.mention} for reason: {reason or 'No reason provided'}")

@npdk1.command()
@commands.has_permissions(ban_members=True)
async def ban(ctx, member: commands.MemberConverter, *, reason=None):
    await member.ban(reason=reason)
    await ctx.send(f"➡️ Banned {member.mention} for reason: {reason or 'No reason provided'}")
...
async def banid(ctx, user_id: int, *, reason=None):
    ...
    if member:
        ...
        await ctx.send("➡️ User not found in the guild.")

@npdk1.command()
@commands.has_permissions(ban_members=True)
async def unban(ctx, user_id: int):
    user = await npdk1.fetch_user(user_id)
...
async def nuke(ctx):
    ...
    await channel.delete()
    await new_channel.send("➡️ This channel has been nuked!")
...
async def name(ctx, *, name: str):
    await ctx.send(f"➡️ Your name is: {name}")

@npdk1.command()
async def nitro(ctx):
    await ctx.send("➡️ Here's a link to Nitro: https://discord.com/nitro")

@npdk1.command()
async def checkprofile(ctx, user: discord.User):
    await ctx.send(f"➡️ Check out {user.mention}'s profile: https://discord.com/users/{user.id}")

@npdk1.command()
async def blurpify(ctx, user: discord.User):
    await ctx.send(f"➡️ {user.mention} has been blurpified! 🗯️")

@npdk1.command()
async def deepfry(ctx, user: discord.User):
    await ctx.send(f"➡️ {user.mention}'s picture has been deep fried! 🔥 (This is a placeholder response)")

@npdk1.command()
async def captcha(ctx, user: discord.User):
    await ctx.send(f"➡️ {user.mention}, please complete the captcha! 🔒 [Link to Captcha](https://example.com/captcha)")

@npdk1.command()
async def threat(ctx, user: discord.User):
    await ctx.send(f"➡️ {user.mention} has issued a threat! ⚠️")

@npdk1.command()
async def iphone(ctx, user: discord.User):
    await ctx.send(f"➡️ {user.mention} just got a new iPhone! 📱")

@npdk1.command()
async def ship(ctx, user: discord.User):
    ships = ["❤️", "💔", "💞", "💓", "💖"]
    await ctx.send(f"➡️ {user.mention}, you are now officially shipped! {random.choice(ships)}")

@npdk1.command(name='ltcqr')
async def lqr(ctx, ltc_ady, usd_amount: float):
    try:
        ...
        await ctx.send(
            f"➡️ Here is your Litecoin QR code for **${usd_amount:.2f} USD** (~{ltc_amount:.8f} LTC):", 
            file=file
        )
    
    except requests.exceptions.RequestException as e:
        await ctx.send(f"➡️ Error fetching exchange rate: {e}")
    except Exception as e:
...
async def setupi(ctx, new_prefix: str):
    ...
    global current_prefix
    if not new_prefix:
        await ctx.send("➡️ Please provide a new Upi.")
        return
    ...
    await ctx.send(f"➡️ Upi updated to: `{new_prefix}`")
...
async def setupi2(ctx, new_prefix: str):
    ...
    global current_prefix
    if not new_prefix:
        await ctx.send("➡️ Please provide a new Upi2.")
        return
    ...
    await ctx.send(f"➡️ Upi2 updated to: `{new_prefix}`")
...
async def setqr(ctx, new_prefix: str):
    ...
    global current_prefix
    if not new_prefix:
        await ctx.send("➡️ Please provide a new Qr.")
        return
    ...
    await ctx.send(f"➡️ Qr updated to: `{new_prefix}`")
...
async def setqr2(ctx, new_prefix: str):
    ...
    global current_prefix
    if not new_prefix:
        await ctx.send("➡️ Please provide a new Qr2.")
        return
    ...
    await ctx.send(f"➡️ Qr2 updated to: `{new_prefix}`")
...
async def setsrvlink(ctx, new_prefix: str):
    ...
    global current_prefix
    if not new_prefix:
        await ctx.send("➡️ Please provide a new Server Link.")
        return
    ...
    await ctx.send(f"➡️ Server Link updated to: `{new_prefix}`")
...
async def setuserid(ctx, new_prefix: str):
    ...
    global current_prefix
    if not new_prefix:
        await ctx.send("➡️ Please provide a new User Id.")
        return
    ...
    await ctx.send(f"➡️ User_Id updated to: `{new_prefix}`")
...
async def setaddy(ctx, new_prefix: str):
    ...
    global current_prefix
    if not new_prefix:
        await ctx.send("➡️ Please provide a new Ltc Addy.")
        return
    ...
    await ctx.send(f"➡️ Ltc Addy updated to: `{new_prefix}`")
...
async def setaddy2(ctx, new_prefix: str):
    ...
    global current_prefix
    if not new_prefix:
        await ctx.send("➡️ Please provide a new Ltc Addy 2.")
        return
    ...
    await ctx.send(f"➡️ Ltc Addy 2 updated to: `{new_prefix}`")
...
async def setbinid(ctx, new_prefix: str):
    ...
    global current_prefix
    if not new_prefix:
        await ctx.send("➡️ Please provide a new Binance id.")
        return
    ...
    await ctx.send(f"➡️ Binance Id updated to: `{new_prefix}`")
...
async def setltckey(ctx, new_prefix: str):
    ...
    global current_prefix
    if not new_prefix:
        await ctx.send("➡️ Please provide a new Ltc key.")
        return
    ...
    await ctx.send(f"➡️ Ltc Key updated to: `{new_prefix}`")
...
@npdk1.command(name='vckick', aliases=['vkick'], brief="Kicks vc user", usage=".vckick <mention.user>")
...
@npdk1.command(name='vcmoveall', aliases=['moveall'], brief="Moves all users to another vc", usage=".vcmoveall <from.channel.id> <to.channel.id>")
async def vcmoveall(ctx, channel1_id: int, channel2_id: int):
    await ctx.message.delete()
    channel1 = npdk1.get_channel(channel1_id)
    channel2 = npdk1.get_channel(channel2_id)
...
@npdk1.command(name='vcmute', aliases=['stfu'], brief="Mutes a vc user", usage=".vcmute <mention.user>")
...
async def vcunmute(ctx, member: discord.Member):
    if ctx.author.voice and member.voice and member.voice.channel == ctx.author.voice.channel:
        ...
        await ctx.send('➡️ User is not in the same voice channel.')

@npdk1.command()
async def vcdeafen(ctx, member: discord.Member):
    if ctx.author.voice and member.voice and member.voice.channel == ctx.author.voice.channel:
        await member.edit(deafen=True)
        await ctx.send(f'➡️ {member.mention} has been deafened.')
    else:
        await ctx.send('➡️ User is not in the same voice channel.')
...
async def vcundeafen(ctx, member: discord.Member):
    if ctx.author.voice and member.voice and member.voice.channel == ctx.author.voice.channel:
        ...
        await ctx.send(f'➡️ {member.mention} has been undeafened.')
    else:
        await ctx.send('➡️ User is not in the same voice channel.')
...
async def vcjoin(ctx, channel: discord.VoiceChannel):
    ...
    await ctx.send(f'➡️ {ctx.author.mention} joined {channel.name}.')

@npdk1.command()
async def vcleave(ctx):
    if ctx.author.voice:
        await ctx.author.move_to(None)
        await ctx.send(f'➡️ {ctx.author.mention} left the voice channel.')
    else:
...
async def vclimit(ctx, limit: int, channel: discord.VoiceChannel = None):
    ...
    await ctx.send(f'➡️ Set the user limit of {limit} for {channel.name}.')
...
async def get_friends():
    relationships = await npdk1.http.get_relationships()
...
async def delallfriends(ctx):
    ...
    while True:
        ...
        await asyncio.gather(*tasks)
        await asyncio.sleep(2)  
    await ctx.send("➡️ **All Friends Have Been Removed**")
...
async def leaveallgroups(ctx):
    await ctx.message.delete()
    for channel in npdk1.private_channels:
...
@npdk1.command(name="hass", description="Random hentai ass")
async def hass(ctx):
    ...
    if image_url:
        ...
        await ctx.send("➡️ An error occurred while fetching the image.")

@npdk1.command(name="ass", description="Random ass")
async def ass(ctx):
...
async def allcmds(ctx):
    ...
    cmds = [command.name for command in npdk1.commands]
    
    # Join the list into a string, separated by commas
    cmds_list = ' ✯ '.join(cmds)
    
    # Send the list to the channel
    await ctx.send(f"➡️ {cmds_list}")
    await ctx.message.delete()
...
@npdk1.command(name='e2u')
async def e2u(ctx, amount: float):
    rates = get_exchange_rate()
    if rates:
        usd_amount = amount * rates['USD']
        await ctx.send(f'**➡️ {amount} EUR is equal to {usd_amount:.2f} USD.**')
        await ctx.message.delete()
    else:
        await ctx.send('**➡️ Error fetching exchange rates.**')

# Command to convert USD to Euro
@npdk1.command(name='u2e')
async def u2e(ctx, amount: float):
    rates = get_exchange_rate()
    if rates:
        euro_amount = amount / rates['USD']
        await ctx.send(f'**➡️ {amount} USD is equal to {euro_amount:.2f} EUR.**')
        await ctx.message.delete()
    else:
        await ctx.send('➡️ Error fetching exchange rates.')
